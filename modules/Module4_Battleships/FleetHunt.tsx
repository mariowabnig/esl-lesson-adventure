...
export default function FleetHunt({ initialMode = "together" }: { initialMode?: "together" | "computer" }) {
  ...
  if (round && Boolean(round.mode === "computer")) return <HuntDuel key={roundId} size={round.size} lives={round.lives} allowDiagonal={round.allowDiagonal} enemy={round.treasures} ships={true} onSetup={() => { setRound(null); setSetupOpen(true); setMessage("Einstellungen wählen und eine neue Runde starten."); }} onRestart={start} />;
  return (
      ...
      {round && (
        <>
          ...
          <p className="text-lg">
            ...
            Jedes Schiff liegt waagrecht oder senkrecht{round.allowDiagonal ? " oder diagonal" : ""}. Schiffe berühren sich nicht, auch nicht an den Ecken.
          </p>
